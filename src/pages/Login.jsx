...
const Login = () => {
  ...
  return (
    <main className='login'>
      {
        token ? (
          <section className='login-logged__cotainer'>
            <i className='login-logged__icon bx bxs-user-circle' ></i>
            <h3 className='login-logged__name'>{firstName} {lastName}</h3>
            <button className='login-logged__btn' onClick={handleLogOut}>Log Out <i className='bx bx-log-out' ></i></button>
          </section>
        ) : (
          <form className='login-form__container' onSubmit={handleSubmit(submit)}>
          <h3 className='loginform__title'>Welcome! Enter your email and password to contunie {"(Test data)"}</h3>
          ...
          <button className='login-form__btn'>Login <i className='bx bx-log-in' ></i></button>
  
          <p className='login-form__footer'>Don't have an account? <span>Sing Up</span></p>
        </form>
        )
      }
    </main>
